...
TEST_BEGIN("Add States")
{
  ...
  int current = fsm.getCurrentState();
  IsTrue("Current State", current == even, "Current state should be EVEN");

	//test stuff with cout's
/*	cout << "Current state is: " << current << endl;
	cout << "Even is: " << even << endl;
	cout << "Odd is: " << odd << endl << endl;
*/
  bool accept = fsm.isAcceptState();
  ...
  IsTrue("Current State", current == odd, "Current state should be ODD");
